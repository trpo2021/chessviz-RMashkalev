...
int rookcheck(char board[9][9], char turn[6])
{
    ...
    if ((turn[0] == turn[3]) && (turn[1] < turn[4]))
        for (i = (asciinumber - turn[4]); i < (asciinumber - turn[1]);
             i++) {
            if ((board[i][(turn[0] - asciiletter)] != ' ') && (turn[4] - turn[1] > 1)) {
                check = 1;
            }
        }
    if ((turn[0] == turn[3]) && (turn[4] < turn[1]))
        for (i = (asciinumber - turn[1]) + 1; i < (asciinumber - turn[4]);
             i++) {
            if (board[i][(turn[0] - asciiletter)] != ' ') {
                check = 1;
            }
        }
    if ((turn[1] == turn[4]) && (turn[3] < turn[0]))
        for (i = (turn[0] - asciiletter) - 1; i < (turn[3] - asciiletter) + 1;
             i++) {
    ...
    if ((turn[1] == turn[4]) && (turn[0] < turn[3]))
        for (i = (turn[3] - asciiletter) - 1; i < (turn[0] - asciiletter) - 1;
             i++) {
            if (board[(asciinumber - turn[1])][i] != ' ') {
                check = 1;
            }
